...
int disable_imu_buffer;
int MAX_ILLEGAL_ACC_CNT=3;
int illegal_acc_cnt=0;
const int MAX_DISABLE_IMU_BUFFER=3;
const int MAX_ACC_UPPERBOUND=30;
const int MAX_ACC_LOWERBOUND=13;
...
void imu_callback(const sensor_msgs::msg::Imu::SharedPtr imu_msg)
{
    ...
    ROS_INFO("Current ACC:%f",acc.norm());
    if (acc.norm() > MAX_ACC_UPPERBOUND)
    {
        illegal_acc_cnt++;
        if (USE_IMU && illegal_acc_cnt >= MAX_ILLEGAL_ACC_CNT)
        {
            estimator.changeSensorType(0, STEREO);
            disable_imu_buffer = MAX_DISABLE_IMU_BUFFER;
            ROS_WARN("Disabling IMU...");
        }
        else
        {
            ROS_WARN("Abnormal Acc Detected! : %f, Dropping",acc.norm());
        }
        return;
    }
    else if(illegal_acc_cnt!=0)
    {
        illegal_acc_cnt--;
    }

    if (disable_imu_buffer !=0)
    {
        if(acc.norm() < MAX_ACC_LOWERBOUND)
            disable_imu_buffer--;
        if (!disable_imu_buffer)
        {
            ROS_WARN("Disable Buffer Cleared, Enabling IMU...");
