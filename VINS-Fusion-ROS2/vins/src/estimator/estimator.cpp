...
void Estimator::clearState(bool is_mProcess_locked, bool use_last_pos)
{
    mBuf.lock();
    if (!is_mProcess_locked)
        mProcess.lock();
    while(!accBuf.empty())
        accBuf.pop_front();
    while(!gyrBuf.empty())
    ...
    auto last_Rs = Rs[WINDOW_SIZE];
    auto last_Ps = Ps[WINDOW_SIZE];
...
void Estimator::setParameter(bool is_mProcess_locked)
{
    if (!is_mProcess_locked)
        mProcess.lock();
    for (int i = 0; i < NUM_OF_CAM; i++)
...
void Estimator::inputIMU(double t, const Vector3d &linearAcceleration, const Vector3d &angularVelocity)
{
    ...
    mBuf.lock();
    accBuf.push_back(make_pair(t, linearAcceleration));
    gyrBuf.push_back(make_pair(t, angularVelocity));
...
void Estimator::processMeasurements()
{
    ...
    while (1)
    {
        ...
        // std::cout<<"DT:"<<(float)(std::chrono::duration<double,std::milli>(now - t).count())<<std::endl;
        // t = now;
        if(!featureBuf.empty())
        {
            // cout << "1" << endl;
            ...
            if(USE_IMU)
            {
                ...
                auto rate = (accVector.size()) / (accVector[accVector.size()-1].first - accVector[0].first);
                cout<<"IMU Samples size: "<<accVector.size()<<endl;
                cout<<"IMU Average Rate: "<<rate<<" Min dt : "<<min_dt<<" Max dt : "<<max_dt<<endl;
                if (std::isinf(rate) || accVector.size() < 4)
                {
                    ROS_WARN("Invalid IMU Samples,skipping this frames...");
                    mProcess.unlock();
                    continue;
                }
            }
...
void Estimator::updateLatestStates()
{
    ...
    latest_gyr_0 = gyr_0;
    mBuf.lock();
    deque<pair<double, Eigen::Vector3d>> tmp_accBuf = accBuf;
    deque<pair<double, Eigen::Vector3d>> tmp_gyrBuf = gyrBuf;
    mBuf.unlock();
    while(!tmp_accBuf.empty())
    {
        ...
        tmp_accBuf.pop_front();
        tmp_gyrBuf.pop_front();
    }
    mPropagate.unlock();
}
