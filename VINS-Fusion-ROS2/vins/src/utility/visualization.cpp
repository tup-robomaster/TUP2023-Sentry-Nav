...
//     // br->sendTransform(tf2::StampedTransform(transform, header.stamp, "odom", "body"));
//     br->sendTransform(tf2::StampedTransform(transform, header.stamp, "odom", "body"));
...
void pubKeyframe(const Estimator &estimator)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        nav_msgs::msg::Odometry odometry;
        odometry.header.stamp = rclcpp::Time(estimator.Headers[WINDOW_SIZE - 2], estimator.Headers[WINDOW_SIZE - 2] - int(estimator.Headers[WINDOW_SIZE - 2]));
        odometry.header.frame_id = "odom";
        odometry.pose.pose.position.x = P.x();
        odometry.pose.pose.position.y = P.y();
        odometry.pose.pose.position.z = P.z();
        ...
        point_cloud.header.stamp = rclcpp::Time(estimator.Headers[WINDOW_SIZE - 2], estimator.Headers[WINDOW_SIZE - 2] - int(estimator.Headers[WINDOW_SIZE - 2]));
