...
public:
    Grid3d(std::string &node_name, rclcpp::Node*nh) : m_cloud(new pcl::PointCloud<pcl::PointXYZ>), m_triGrid(NULL)
    {
        // Load paraeters
        double value;
        m_nh = nh;
        ...
        nh->declare_parameter("global_frame_id", std::string("map"));
        nh->declare_parameter("map_path", std::string("map.ot"));
        nh->declare_parameter("publish_grid_slice", 1.0);
        nh->declare_parameter("publish_point_cloud_rate", 0.2);
        nh->declare_parameter("publish_grid_slice_rate", 0.2);
        nh->declare_parameter("publish_point_cloud", false);

        nh->get_parameter("global_frame_id", m_globalFrameId);
        nh->get_parameter("map_path", m_mapPath);
        nh->get_parameter("publish_point_cloud", m_publishPc);
        ...
        if(loadOctomap(m_mapPath))
        {
            ...
            std::cout<<m_publishGridSliceRate<<std::endl;
            std::cout<<m_publishPointCloudRate<<std::endl;
            // Build the msg with a slice of the grid if needed
            if(m_gridSlice >= 0 && m_gridSlice <= m_maxZ)
            {
                buildGridSliceMsg(m_gridSlice);
                m_gridSlicePub = m_nh->create_publisher<nav_msgs::msg::OccupancyGrid>(node_name+"/grid_slice", 1);
                gridTimer = rclcpp::create_timer(m_nh,
                                m_nh->get_clock(),
                                rclcpp::Duration::from_seconds(1.f / m_publishGridSliceRate),
                                std::bind(&Grid3d::publishGridSlice, this));
                // m_gridSlicePub = m_nh.advertise<nav_msgs::msg::OccupancyGrid>(node_name+"/grid_slice", 1, true);
            ...
            if(m_publishPc)
            {
                m_pcPub = m_nh->create_publisher<sensor_msgs::msg::PointCloud2>(node_name+"/map_point_cloud", 1);
                mapTimer = rclcpp::create_timer(m_nh,
                                m_nh->get_clock(),
                                rclcpp::Duration::from_seconds(1.f / m_publishPointCloudRate),
                                std::bind(&Grid3d::publishMapPointCloud, this));
        ...
        {
            std::cerr<<"Map load failed! "<<m_mapPath<<std::endl;
        }
    ...
    Grid3d(std::string &node_name, std::string &map_path) : m_cloud(new pcl::PointCloud<pcl::PointXYZ>), m_triGrid(NULL)
    {
        ...
        m_ndt.setTransformationEpsilon (0.01);  // Setting minimum transformation difference for termination condition.
        m_ndt.setStepSize (0.1);   // Setting maximum step size for More-Thuente line search.
        m_ndt.setResolution (0.2);   //Setting Resolution of NDT grid structure (VoxelGridCovariance).
        m_ndt.setMaximumIterations (50);   // Setting max number of registration iterations.
    }
    ...
    {
        m_pcMsg.header.stamp = m_nh->now();
        m_pcPub->publish(m_pcMsg);
    }
    
    void publishGridSlice(void)
    {
        m_gridSliceMsg.header.stamp = m_nh->now();
    ...
    bool alignICP(std::vector<pcl::PointXYZ> &p, double &tx, double &ty, double &tz, double &roll, double &pitch, double &yaw)
    {
        ...
        auto has_converged = m_icp.hasConverged();
        auto fit_error = m_icp.getFitnessScore();
        if (!has_converged || fit_error > MAX_FIT_MSE_ERROR)
        {
            RCLCPP_WARN(m_nh->get_logger(), "ICP ERROR,Skip this frame for hasConverged: %d, Fit MSE: %f", has_converged, fit_error);
            return false;
        }
        // Get solution
    ...
    bool alignNDT(std::vector<pcl::PointXYZ> &p, double &tx, double &ty, double &tz, double &roll, double &pitch, double &yaw)
    {
        pcl::PointCloud<pcl::PointXYZ>::Ptr c(new pcl::PointCloud<pcl::PointXYZ>);
        pcl::PointCloud<pcl::PointXYZ> Final;
        ...
        m_ndt.setInputSource(c);

        Eigen::AngleAxisf initRotation(yaw, Eigen::Vector3f::UnitZ ());
        Eigen::Translation3f initTranslation (tx, ty, tz);
        Eigen::Matrix4f initGuess = (initTranslation * initRotation).matrix ();

        // Setup NDT and perform alignement
        m_ndt.align(Final, initGuess);

        auto has_converged = m_ndt.hasConverged();
        auto fit_error = m_ndt.getFitnessScore();
        if (!has_converged || fit_error > MAX_FIT_MSE_ERROR)
        {
            RCLCPP_WARN(m_nh->get_logger(), "NDT ERROR, Skip this frame for hasConverged: %d, Fit MSE: %f", has_converged, fit_error);
            return false;
        }
        // // Get solution
...
protected:
    ...
    void computePointCloud(void)
    {
        ...
        int i = 0;
        pcl::PointCloud<pcl::PointXYZ>::Ptr m_cloud_out(new pcl::PointCloud<pcl::PointXYZ>);
        m_cloud->width = m_octomap->size();
        m_cloud->height = 1;
        m_cloud->points.resize(m_cloud->width * m_cloud->height);

        m_cloud_out->width = m_octomap->size();
        m_cloud_out->height = 1;
        m_cloud_out->points.resize(m_cloud_out->width * m_cloud_out->height);
        for(octomap::OcTree::leaf_iterator it = m_octomap->begin_leafs(), end = m_octomap->end_leafs(); it != end; ++it)
        {
            if(it != NULL && m_octomap->isNodeOccupied(*it))
            {
                ...
                m_cloud_out->points[i].x = it.getX();
                m_cloud_out->points[i].y = it.getY();
                m_cloud_out->points[i].z = it.getZ();
                i++;
            }
        }
        m_cloud->width = i;
    ...
    void buildGridSliceMsg(float z)
    {
        ...
        m_gridSliceMsg.info.map_load_time = m_nh->now();
