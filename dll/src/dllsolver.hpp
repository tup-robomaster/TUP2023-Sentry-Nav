...
class DLLSolver
{
  ...
  public:
    ...
    bool solve(std::vector<pcl::PointXYZ> &p, double &tx, double &ty, double &tz, double &yaw)
    {
        ...
        {
            if (_grid.isIntoMap(p[i].x, p[i].y, p[i].z))
            {
                CostFunction* cost_function = new DLLCostFunction(p[i].x, p[i].y, p[i].z, _grid);
                problem.AddResidualBlock(cost_function, new ceres::HuberLoss(0.1), x); 
            }
            // else
            // {
            //     std::cout<<"Out :"<<p[i].x<<" "<<p[i].y<<" "<<p[i].z<<std::endl;
            // }
        }
        ...
        std::cout<<summary.num_residual_blocks<<std::endl;
        std::cout<<summary.BriefReport()<<std::endl;
        std::cout<<"T: "<<summary.inner_iteration_time_in_seconds<<std::endl;
        // Get the solution
