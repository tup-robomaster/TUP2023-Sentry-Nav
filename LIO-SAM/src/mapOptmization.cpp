...
public:
    ...
    void updateInitialGuess()
    {
        ...
        {
            // RCUTILS_LOG_INFO("Init R: %f, P: %f, Y: %f", cloudInfo.initial_guess_roll, cloudInfo.initial_guess_pitch, cloudInfo.initial_guess_yaw);
    ...
    void publishOdometry()
    {
        ...
        if(lidarFrame != baselinkFrame)
        {
            try
            {
                tf2::fromMsg(tfBuffer->lookupTransform(
                    lidarFrame, baselinkFrame, timeLaserInfoStamp, rclcpp::Duration::from_seconds(0.01)), lidar2Baselink);
            }
        ...
        tfBroadcaster->sendTransform(trans_odom_to_base);        

        // Publish odometry for ROS (incremental)
        static bool lastIncreOdomPubFlag = true;
